...
if (isset($_GET['delete_room'])) {
    ...
    // Hapus data di tabel booking yang memiliki room_id yang sama
    $sql_booking = "DELETE FROM booking WHERE room_id = '$room_id'";
    $result_booking = mysqli_query($connection, $sql_booking);

    // Jika penghapusan di tabel booking berhasil, lanjutkan untuk menghapus baris di tabel room
    if ($result_booking) {
...
if (isset($_POST['room_type'])) {
    ...
    $sql = "SELECT * FROM room WHERE room_type_id = '$room_type_id' AND status IS NULL AND check_in_status = '0'";
    $result = mysqli_query($connection, $sql);
    if ($result) {
...
if (isset($_POST['change_shift'])) {
    // Pastikan koneksi ke database telah dibuat di atas

    $emp_id = $_POST['emp_id'];
    $shift_id = $_POST['shift_id'];

    // Update emp_history to set the to_date for the current shift
    ...
    if ($update_result) {
        // Insert a new emp_history record for the changed shift
        $insert = "INSERT INTO emp_history (emp_id, shift_id) VALUES ('$emp_id', '$shift_id')";
        $insert_result = mysqli_query($connection, $insert);

        // Update staff to set the new shift
        $update_staff = "UPDATE staff SET shift_id = '$shift_id' WHERE emp_id = '$emp_id'";
        $update_staff_result = mysqli_query($connection, $update_staff);

        if ($insert_result && $update_staff_result) {
            // Jika semua operasi berhasil, arahkan kembali ke halaman index.php?staff_mang
            header("Location: index.php?staff_mang");
            exit(); // Penting untuk menghentikan eksekusi kode selanjutnya
        } else {
            $error_message = 'Error changing shift';
        }
    } else {
        $error_message = 'Error updating emp_history';
    }
}
